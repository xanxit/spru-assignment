        ...
        <div className='max-w-[540px] sm:max-w-[720px] md:max-w-[960px] lg:max-w-[1200px] mx-auto px-[15px]'>
            <div className='flex flex-col sm:flex-row gap-[30px]'>
                <div className='w-full sm:w-[33%] shrink-0 '>
                    <div className= "flex flex-nowrap overflow-x-hidden snap-x snap-mandatory" ref={carouselRef}>
                        {(product.image).map((img,index)=>{
                            return <img src={img} alt="" key={index} className='w-full h-auto snap-center'/>
                        })}
                    </div>
                </div>
                <div>
                    <div>
                    <h1 className='uppercase font-[500] text-[30px]'>{product.title}</h1>
                    ...
                    <div>
                        ...
                        <ul className='mb-[24px] font-lato list-disc px-[20px]'>
                            {(product.benefit).map((point, index) => {
                                return <li className='py-[5px]' key={index}>{point}</li>
                            })}
                        </ul>
                    </div>
                    <div>
                        ...
                        <ul className='list-disc px-[20px]'>
                            {(product.availability).map((size,index) => {
                                return <li className='py-[5px]' key={index}>{size}</li> 
                            })}
                        </ul>
                    </div>
                </div>
